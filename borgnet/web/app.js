...
function conferenceBar(items){
  ...
  const group={root,body,models,preview,statuses:new Map(),streams:new Map(),follow:true};
  ...
  items.forEach(item=>{const id=item.connection||item.id;if(group.statuses.has(id))return;const chip=el('span','model-status'),name=el('span','model-name',item.model),status=el('span','model-working','Thinking');chip.title=item.address||item.url||'';chip.append(name,status);models.append(chip);group.statuses.set(id,{chip,status});});
  return group;
}
...
function modelStatus(group,result,status){const item=group.statuses.get(result.connection);if(!item)return;item.chip.dataset.status=status;item.status.textContent=status==='thinking'?'Thinking':status==='error'?'Error':status==='stopped'?'Stopped':'';}
function resultCard(result,group){const final=result.phase==='final'||result.synthesis;const card=el('article',`result${final?' synthesis':''}`),head=el('header'),name=el('div');name.append(el('strong','',`${final?'Unified answer · ':result.phase==='review'?'Peer review · ':result.phase==='proposal'?'Proposal '+result.proposal_id+' · ':result.phase?.startsWith('argument-')?'Argument R'+result.revision+' · ':result.phase?.startsWith('challenge-')?'Challenge R'+result.revision+' · ':result.phase==='implementation'?'Implementation · ':''}${result.model}`),el('small','',result.address));head.append(name,el('span','health','Thinking…'));card.append(head,el('div','answer','Waiting for a response…'));card.conferenceGroup=group;group.body.append(card);return card;}
function finishCard(card,result){if(card.conferenceGroup&&(result.phase==='final'||!card.conferenceGroup.streams.size))streamPreview(card.conferenceGroup,{...result,text:result.text||result.error||'Complete'});if((result.phase==='final'||result.synthesis)&&card.conferenceGroup)card.conferenceGroup.root.insertAdjacentElement('afterend',card);card.querySelector('.answer').textContent=result.text||result.error||'No response';card.querySelector('.answer').classList.toggle('error',result.status==='error');card.querySelector('.health').textContent=result.status==='error'?'Error':result.status==='partial'?'Partial participation':result.seconds!==undefined?`${result.seconds}s`:'Complete';if(result.text)card.querySelector('header,summary').append(button('Copy',async()=>{await navigator.clipboard.writeText(result.text);toast('Copied response.');}));}
async function sendPrompt(event){event?.preventDefault();const prompt=$('#prompt').value.trim();if(!prompt||busy)return;if(!selected().length)return toast('Connect a provider and choose a model first.');if($('#feed .welcome'))$('#feed').replaceChildren();addPrompt(prompt);$('#prompt').value='';const ids=selected().map(c=>c.id), synthesis=$('#synthesis').disabled?'':$('#synthesis').value;busy=true;controller=new AbortController();renderConnections();tab('conversation');const cards=new Map(),group=conferenceBar(selected());try{const response=await fetch('/api/dispatch',{method:'POST',headers:{'Content-Type':'application/json','X-BorgNet-Token':state.token},body:JSON.stringify({prompt,connections:ids,contexts:[...attached],conversation,synthesize:synthesis==='__independent__'?'':synthesis,collaborate:synthesis!=='__independent__',collaboration_mode:$('#collaborationMode').value,debate_rounds:Number($('#debateRounds').value),implement:$('#collaborationMode').value==='debate'&&$('#debateImplement').getAttribute('aria-checked')==='true',executor:$('#debateExecutor').value}),signal:controller.signal});if(!response.ok){const error=await response.json();throw new Error(error.error||error.detail||'Request failed');}const reader=response.body.getReader(),decoder=new TextDecoder();let buffer='';while(true){const {value,done}=await reader.read();buffer+=decoder.decode(value||new Uint8Array(),{stream:!done});let newline;while((newline=buffer.indexOf('\n'))>=0){const line=buffer.slice(0,newline);buffer=buffer.slice(newline+1);if(!line)continue;const message=JSON.parse(line),key=message.connection+'-'+(message.phase||'response')+(message.synthesis?'-synthesis':'');if(message.type==='run')conversation=message.conversation;if(message.type==='phase')$('#status').textContent=message.text;if(message.type==='started'){modelStatus(group,message,'thinking');cards.set(key,resultCard(message,group));}if(message.type==='stream-reset'){streamPreview(group,message,true);const card=cards.get(key)||resultCard(message,group);cards.set(key,card);card.querySelector('.answer').textContent='';card.querySelector('.health').textContent=message.mode==='buffered'?'Waiting · CLI returns completed messages':'Streaming…';}if(message.type==='delta'){streamPreview(group,message);const card=cards.get(key)||resultCard(message,group);cards.set(key,card);card.querySelector('.answer').append(document.createTextNode(message.text));card.querySelector('.health').textContent='Receiving…';}if(message.type==='result'){const card=cards.get(key)||resultCard(message,group);cards.set(key,card);finishCard(card,message);modelStatus(group,message,message.status==='error'?'error':'complete');}if(!group.root.open)$('#feed').scrollTop=$('#feed').scrollHeight;}if(done)break;}}catch(error){const stopped=error.name==='AbortError';for(const [id,item] of group.statuses)if(item.status.textContent==='Thinking')modelStatus(group,{connection:id},stopped?'stopped':'error');toast(stopped?'Stopped waiting. A remote provider may continue an already accepted request.':error.message);for(const card of cards.values())if(card.querySelector('.health').textContent==='Thinking…'){card.querySelector('.health').textContent=stopped?'Stopped':'Interrupted';card.querySelector('.answer').textContent=stopped?'Request cancelled in this workspace.':'Connection interrupted before a response arrived.';}if(!stopped)$('#prompt').value=prompt;}finally{busy=false;controller=null;await load();}}
...
function updateDebateControls(){
  ...
  const independent=$('#synthesis').value==='__independent__';
...
$('#synthesis').addEventListener('change',updateDebateControls);
