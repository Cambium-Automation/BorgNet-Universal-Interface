...
function tab(name) { if (!document.getElementById(name + 'Panel')) return; try { localStorage.setItem('borgnet-active-tab', name); } catch {} $$('.tabs button').forEach(b => { b.classList.toggle('active', b.dataset.tab === name); b.setAttribute('aria-pressed', String(b.dataset.tab === name)); }); $$('.panel').forEach(p => p.classList.toggle('active', p.id === name + 'Panel')); window.dispatchEvent(new CustomEvent('borgnet-tab-change', {detail:name})); }
...
function conferenceBar(items){
  ...
  summary.append(label,models);root.append(summary,body);$('#feed').append(root);
  root.open=true;const group={root,body,models,statuses:new Map()};
...
function resultCard(result,group){const final=result.phase==='final'||result.synthesis;const card=el('article',`result${final?' synthesis':''}`),head=el('header'),name=el('div');name.append(el('strong','',`${final?'Unified answer · ':result.phase==='review'?'Peer review · ':result.phase==='proposal'?'Proposal '+result.proposal_id+' · ':''}${result.model}`),el('small','',result.address));head.append(name,el('span','health','Thinking…'));card.append(head,el('div','answer','Waiting for a response…'));if(final)group.root.insertAdjacentElement('afterend',card);else group.body.append(card);return card;}
